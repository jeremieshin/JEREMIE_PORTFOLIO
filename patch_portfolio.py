...
css = r'''
  ...
  .burger{
    ...
    color:#777777!important;mix-blend-mode:normal!important;opacity:1!important;visibility:visible!important;pointer-events:auto!important;
  }
  ...
  body.inverted .burger.open{color:#777777!important;}
  ...
  /* Option 03: minimal line theme toggle with a soft moon-to-sun morph. */
  .global-theme-toggle{
    position:fixed!important;right:40px!important;bottom:72px!important;z-index:390!important;
    width:42px!important;height:42px!important;min-width:42px!important;min-height:42px!important;
    padding:0!important;margin:0!important;border:1px solid #777777!important;border-radius:0!important;
    background:transparent!important;color:#777777!important;
    display:flex!important;align-items:center!important;justify-content:center!important;box-sizing:border-box!important;
    cursor:pointer!important;
  }
  .global-theme-toggle svg{width:24px!important;height:24px!important;display:block!important;overflow:visible!important;}
  .global-theme-toggle .theme-label{display:none!important;}
  .global-theme-toggle .theme-moon,
  .global-theme-toggle .theme-sun{
    transform-box:fill-box;
    transform-origin:center;
    transition:opacity .36s ease,transform .5s cubic-bezier(.16,.8,.24,1);
  }
  .global-theme-toggle .theme-moon{opacity:1;transform:rotate(0deg) scale(1);}
  .global-theme-toggle .theme-sun{opacity:0;transform:rotate(-40deg) scale(.72);}
  body.inverted .global-theme-toggle{color:#777777!important;}
  body.inverted .global-theme-toggle .theme-moon{opacity:0;transform:rotate(35deg) scale(.72);}
  body.inverted .global-theme-toggle .theme-sun{opacity:1;transform:rotate(0deg) scale(1);}
  .global-theme-toggle:hover svg{transform:scale(1.06);transition:transform .22s ease;}
  .global-theme-toggle:active svg{transform:scale(.94);}
  ...
  @media(max-width:900px){.work-detail-image{width:calc(100vw - 56px)!important;}}
...
(() => {
  ...
  ready(() => {
    ...
    const lockMenuScroll = () => { body.classList.add('menu-open'); root.classList.add('menu-open'); };
    const unlockMenuScroll = () => {
      body.classList.remove('menu-open'); root.classList.remove('menu-open');
      body.style.position='';body.style.top='';body.style.left='';body.style.right='';body.style.width='';body.style.overflow='';root.style.overflow='';
    };
    unlockMenuScroll();
    ...
    if (oldBurger && menu) {
      ...
      setMenu(false);
      burger.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();setMenu(!menu.classList.contains('open'));});
      document.querySelectorAll('.menu-link,.menu-nav a').forEach(link=>link.addEventListener('click',()=>setMenu(false)));
    ...
    /* One button only. Minimal moon and sun share one center and cross-morph in place. */
    ...
    if(theme){
      ...
      theme.innerHTML=`
        <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
          <g class="theme-moon" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round">
            <path d="M16.7 15.9A7.15 7.15 0 0 1 8.1 7.3a6.65 6.65 0 1 0 8.6 8.6Z"/>
          </g>
          <g class="theme-sun" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round">
            <circle cx="12" cy="12" r="3.55"/>
            <path d="M12 2.6v2.15M12 19.25v2.15M2.6 12h2.15M19.25 12h2.15M5.35 5.35l1.52 1.52M17.13 17.13l1.52 1.52M18.65 5.35l-1.52 1.52M6.87 17.13l-1.52 1.52"/>
          </g>
        </svg>`;
    ...
    overlay.id='workDetailOverlay';overlay.className='work-detail-overlay';overlay.setAttribute('aria-hidden','true');
    ...
    const openDetail=()=>{
      ...
      if(menu)menu.classList.remove('open');if(burger)burger.classList.remove('open');unlockMenuScroll();overlay.scrollTop=0;
      body.classList.add('detail-open');root.classList.add('detail-open');overlay.classList.add('open');overlay.setAttribute('aria-hidden','false');
    };
    const closeDetail=()=>{
      overlay.classList.remove('open');overlay.setAttribute('aria-hidden','true');body.classList.remove('detail-open');root.classList.remove('detail-open');
      body.style.overflow='';root.style.overflow='';overlay.scrollTop=0;requestAnimationFrame(()=>window.scrollTo(0,detailScrollY));
    };
    const grid=document.querySelector('.grid');
    if(grid)grid.addEventListener('click',e=>{const tile=e.target.closest('.tile');if(!tile||tile!==grid.querySelector('.tile'))return;e.preventDefault();e.stopPropagation();openDetail();});
    closeBtn.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();closeDetail();});
