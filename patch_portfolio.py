...
css = r'''
  ...
  .filter-btn{font-size:90%!important;font-weight:800!important;}
  ...
  /* Keep the same square button in both menu states; only the two inner lines morph. */
  .burger,.menu-close{width:42px!important;height:42px!important;border:1px solid currentColor!important;border-radius:0!important;background:transparent!important;display:flex!important;align-items:center!important;justify-content:center!important;flex-direction:column!important;gap:7px!important;padding:0!important;box-sizing:border-box!important;}
  .burger span,.menu-close span{display:block!important;width:20px!important;height:2px!important;background:currentColor!important;margin:0!important;transform-origin:center!important;transition:transform .48s cubic-bezier(.16,.8,.24,1),opacity .25s ease!important;}
  ...
  /* Theme toggle remains available on the closed/main screen at the same viewport position. */
  .global-theme-toggle{position:fixed!important;right:40px!important;bottom:40px!important;z-index:390!important;}
  ...
  /* First Work card detail: full viewport backdrop, image floating above it. */
  ...
  @media(max-width:700px){.global-theme-toggle{right:20px!important;bottom:24px!important}.work-detail-stage{padding:82px 14px 40px}.work-detail-image{width:94vw}.work-detail-close{top:20px;right:20px}}
...
(() => {
  ...
  ready(() => {
    ...
    const menuTheme = document.querySelector('.theme-toggle');
    if (menuTheme && !document.querySelector('.global-theme-toggle')) {
      const globalTheme = menuTheme.cloneNode(true);
      globalTheme.removeAttribute('id');
      globalTheme.classList.add('global-theme-toggle');
      document.body.appendChild(globalTheme);
      globalTheme.addEventListener('click', () => {
        body.classList.toggle('inverted');
        const isInv = body.classList.contains('inverted');
        const label = globalTheme.querySelector('.theme-label');
        if (label) label.textContent = isInv ? 'Light Mode' : 'Dark Mode';
      });
    }
